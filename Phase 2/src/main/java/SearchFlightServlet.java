...
request.getParameter("destination_city"); 
 String date_travel = request.getParameter("date_travel"); 
 try { 
 SearchFlightDao dao = new SearchFlightDao(); 
 List<String[]> flights = 
dao.getAvailableFlights(source_city, destination_city, date_travel); 
 ...
 * String source_city = request.getParameter("source_city"); 
String 
 * date_travel = request.getParameter("date_tavel"); //String 
noPerson 
 ...
 * 
 * try { if (searchFlightDao.validate(searchFlightBean)) { 
HttpSession 
...
out.println("hello"); 
 * response.sendRedirect("flight_details.jsp"); } else { 
//HttpSession 
 * session = request.getSession(); 
